...
require_once('../../server/checkadmin.php');
    ...
    if(isset($_POST['submit'])){
        ...
        try { 
            ...
            $stmt = $conn->prepare("UPDATE teachers SET username = :username, prefix = :prefix, teacher_name = :teacher_name,
            teacher_surname = :teacher_surname WHERE teacher_id = :teacher_id");
...
แก้ไขข้อมูลผู้ใช้งานอาจารย์ </H4>
        <hr>
        <form method="post">
        ...
        <input type="hidden" name="id" value="<?php echo $row['teacher_id']?>">  
            ...
            <div class="form-group mb-3">
                ...
                <input class="form-control" type="text"name="teacher_name" value="<?php echo $row['teacher_name']?>" placeholder="ป้อนชื่อ" required>
            </div>
            
            <div class="form-group mb-3">
                <label for="teacher_surname" class="form-label">นามสกุล</label>
                <input class="form-control" type="text"name="teacher_surname" value="<?php echo $row['teacher_surname']?>" placeholder="ป้อนนามสกุล" required>
            </div>
