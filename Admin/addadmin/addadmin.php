...
<?php
 ...
 if(isset($_POST['submit'])){
     ...
     $passwordHashed = password_hash($password, PASSWORD_DEFAULT);       
      ...
      $sqlUser = "SELECT * FROM `admins` WHERE `username` = '".$username."'";
        $checkUser = $conn->query($sqlUser);
        if( !$checkUser->rowCount() ) {
            $sql = "INSERT INTO `admins` (`username`, `password`, `prefix`, `admin_name`, `admin_surname`, `license`) 
            VALUES ('".$username."', 
                    '".$passwordHashed."', 
                    '".$_POST['prefix']."', 
                    '".$_POST['admin_name']."', 
...
<head>
    ...
    <title>เพิ่มข้อมูลผู้ดูแล</title>
...
<h4><i class="fas fa-user-plus"></i> เพิ่มข้อมูลผู้ใช้ผู้ดูแล</h4>
    <hr>
        <form action=" " method="post">
            ...
            <div class="form-group mb-3">
                ...
                <input class="form-control" type="text"name="admin_name" placeholder="ป้อนชื่อ" required>
            </div>
            
            <div class="form-group mb-3">
                <label for="admin_surname" class="form-label">นามสกุล</label>
                <input class="form-control" type="text"name="admin_surname" placeholder="ป้อนนามสกุล" required>
            </div>
